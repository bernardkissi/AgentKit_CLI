export type TokenType =
    | "ident" | "number" | "string"
    | "op" | "dot" | "lparen" | "rparen" | "lbrack" | "rbrack";

export interface Token { type: TokenType; value: string; pos: number; }

const ILLEGAL = /[;`{}]/;

export function tokenize(expr: string): { ok: true; tokens: Token[] } | { ok: false; error: string } {
    if (ILLEGAL.test(expr)) return { ok: false, error: "Illegal character in expression" };
    ...
    while (i < s.length) {
        ...
        if (c === ".") { tokens.push({ type: "dot", value: ".", pos: i }); i++; continue; }
        if (c === "(") { tokens.push({ type: "lparen", value: "(", pos: i }); i++; continue; }
        if (c === ")") { tokens.push({ type: "rparen", value: ")", pos: i }); i++; continue; }
        if (c === "[") { tokens.push({ type: "lbrack", value: "[", pos: i }); i++; continue; }
        if (c === "]") { tokens.push({ type: "rbrack", value: "]", pos: i }); i++; continue; }
        ...
        if (c === "'" || c === '"') {
            ...
            if (j >= s.length) return { ok: false, error: "Unterminated string literal" };
        ...
        if (isAlpha(c)) {
            ...
            if (forbidden.has(ident)) return { ok: false, error: `Forbidden keyword: ${ident}` };
        ...
        const ops2 = new Set(["==", "!=", "<=", ">=", "&&", "||"]);
        if (ops2.has(two)) { tokens.push({ type: "op", value: two, pos: i }); i += 2; continue; }
        const ops1 = new Set(["+", "-", "*", "/", "%", "<", ">", "!"]);
        if (ops1.has(c)) { tokens.push({ type: "op", value: c, pos: i }); i++; continue; }
